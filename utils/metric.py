...
class Evaluator(object):
    ...
    def get_tp_fp_tn_fn(self):
        """
        该方法假设self对象具有一个名为confusion_matrix的属性，它是一个二维的NumPy数组。
        具体地，代码中的计算如下：

        tp计算了混淆矩阵的对角线元素，即每个类别被正确分类的样本数。
        fp计算了每个类别的列总和减去对角线元素，表示被错误分类为该类别的样本数。
        fn计算了每个类别的行总和减去对角线元素，表示被错误分类为其他类别的样本数。
        tn计算了混淆矩阵对角线元素的总和减去对角线元素，表示被正确分类为其他类别的样本数。
    ...
    def Frequency_Weighted_Intersection_over_Union(self):
        """
        频权交并比（Frequency Weighted Intersection over Union）。它结合了每个类别的频率（Frequency）和对应的交并比（Intersection over Union）。

        首先，使用np.sum(self.confusion_matrix, axis=1)对混淆矩阵的行进行求和，得到每个类别的像素总数。
        然后，将每个类别的像素总数除以混淆矩阵中所有元素的总和（即总像素数），加上一个很小的数self.eps，以避免除以零的情况。这样得到每个类别的频率（Frequency）。
    ...
    def add_batch(self, gt_image, pre_image):
        """
        用于将一批（batch）的真实标签图像和预测标签图像添加到混淆矩阵中。
        """
