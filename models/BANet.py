...
以防止过拟合。它的输入是大小为in_features的张量，输出是大小为out_features的张量。如果未指定hidden_features，
则默认与in_features相同。
...
"""
这是一个Transformer中的Block，包含了一个Attention和一个Feed-Forward网络（Mlp）。
...
"""
位置编码模块 Position
这个位置编码器的具体实现方式是，在卷积层中，每个通道只和自己的空间位置相关，而不同通道之间的信息相互独立。
这样就可以根据每个位置的通道信息，学习到该位置的语义信息，从而更好地提取图像特征。
...
"""
这段代码定义了一个名为GL的PyTorch模块，它实现了一个具有通道组卷积的残差连接。
该模块的构造函数接受一个整数参数dim，表示输入和输出张量的通道数。
...
"""
PatchEmbed模块是将输入的图像按照指定的patch_size分割成若干个块，并将每个块进行卷积和标准化，最后展平成一个向量。
...
"""
这是一个名为 ResT 的 PyTorch 模块，它实现了一个由四个阶段组成的 ViT 模型。
其输入张量的通道数为3，输出分类数为1000。模型的特征维度分别为[64, 128, 256, 512]，
...
def rest_lite(pretrained=True, weight_path='pretrain_weights/rest_lite.pth', **kwargs):
    # pretrained=True pretrain_weights/rest_lite.pth
...
"""
这是一个定义了 Convolution-BatchNorm-ReLU 模块的 PyTorch 类。
...
首先，通过调用LinearAttention模块，使用注意力机制计算高层特征的注意力矩阵。
接下来，通过ConvBNReLU模块进行卷积和批量归一化，将注意力矩阵转换为与低层特征相同的通道数。
最后，使用UpSample模块将通道数进行上采样，并将其与低层特征相乘，形成注意力嵌入。最终输出结果为低层特征与注意力嵌入相加的结果。
"""
...
"""
这段代码实现了一个纹理路径模块，主要用于提取图像的纹理信息。
模块包括四个卷积层，其中前三个卷积层的步长为2，用于下采样输入图像。最后一个卷积层用于输出128个特征图。
在初始化时，使用kaiming_normal_方法对卷积层的权重进行初始化。get_params方法用于获取模块中的权重和偏置参数。
"""
...
"""
这是一个名为DependencyPathRes的类，它继承了PyTorch的nn.Module类。
...
"""
这是一个基于BANet模型的类定义，用于进行6类图像分类。该模型由以下几个模块组成：
